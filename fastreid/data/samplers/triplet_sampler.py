...
class BalancedIdentitySampler(Sampler):
    ...
    def __iter__(self):
        start = self._rank
        # itertools.islice(可迭代对象, 起点, 终点, 切片数量), 
        # 替代了distributed.DistributedSampler()的功能：即不同GPU加载不同的数据
    ...
    def _infinite_indices(self):
        ...
        while True:
            ...
            while len(avai_pids) >= self.num_pids_per_batch:
                ...
                # yield from 是在Python3.3才出现的语法,一个简单应用是拼接可迭代对象，消除内层for循环
                # 比如要展开['ABC', [1,2,3]]得到['A','B','C',1,2,3]，一种是两个for循环+yield；一种是一个for循环+yield from
