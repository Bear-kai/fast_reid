...
class Baseline(nn.Module):
    ...
    def losses(self, outs):
        ...
        # model predictions
        pred_class_logits = outputs['pred_class_logits'].detach()   # 用原logits算预测精度
        cls_outputs       = outputs['cls_outputs']                  # 用（可能）被修改的logits算分类损失
        pred_features     = outputs['features']
        # fmt: on

        # Log prediction accuracy
        log_accuracy(pred_class_logits, gt_labels)                  # 记录train_acc到storage中
