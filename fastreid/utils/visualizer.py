...
class Visualizer:
    ...
    def get_model_output(self, all_ap, dist, q_pids, g_pids, q_camids, g_camids):
        ...
        # 2020.10.25加; 2020.10.26去掉,否则rank_list第二排，可能没有正确样本用于显示
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
                         actmap=False):
        if vis_label:
            fig, axes = plt.subplots(2, max_rank + 1, figsize=(3 * max_rank, 12))  # 这里先定画布(fig)大小, axes用不上
        else:                                                                      # plt.subplots()是一次性分好了所有区域
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            plt.clf()                                                               # 这里先清理刚才画布的分区(画布的大小其实保留了)
            ax = fig.add_subplot(1, max_rank + 1, 1)                                # 重新添加分区(单个添加)，这里对应展示query图片的区域
            ax.imshow(query_img)
            ...
            for i in range(max_rank):
                if vis_label:
                    ax = fig.add_subplot(2, max_rank + 1, i + 2)                    # 这里继续添加分区(单个添加)
                else:
                ...
                if cmc[i] == 1:
                    label = 'true'
                    ax.add_patch(plt.Rectangle(xy=(0, 0), width=gallery_img.shape[1] - 1,             # add_patch将形状添加到ax分区(或称子图)中 
            ...
            if vis_label:
                ...
                # 这里ascending是对sim得分而言，而索引号大的sim小，因此需要索引倒序，实现sim的ascending
                ...
                for i in range(max_rank):
                    ...
                    ax.add_patch(plt.Rectangle(xy=(0, 0), width=gallery_img.shape[1] - 1,
                                               ...
                                               fill=False, linewidth=5))
                    ax.imshow(gallery_img)
                    ax.set_title(f'{self.sim[q_idx, sort_idx[j]]:.3f}/{str(j)}/cam{cam_id}')     # 添加正确结果的索引号
                    ax.axis("off")
    ...
    def vis_rank_list(self, output, vis_label, num_vis=100, rank_sort="ascending", label_sort="ascending", max_rank=5,
        ...
        # 改为显示一部分AP高的query, 显示一部分AP低的query
        query_indices_lowAP = query_indices[:num_vis//2]
        query_indices_highAP = query_indices[::-1][:num_vis//2]
        output_dir_low = os.path.join(output,'lowAP')
        output_dir_high = os.path.join(output,'highAP')
        if not os.path.exists(output_dir_low):
            os.makedirs(output_dir_low)
        if not os.path.exists(output_dir_high):
            os.makedirs(output_dir_high)
        self.save_rank_result(query_indices_lowAP, output_dir_low, max_rank, vis_label, label_sort, actmap)
        self.save_rank_result(query_indices_highAP, output_dir_high, max_rank, vis_label, label_sort, actmap)
