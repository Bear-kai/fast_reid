...
class PeriodicCheckpointer:
    ...
    def __init__(self, checkpointer: Any, period: int, max_iter: int = None):
        """
        Args:
            checkpointer (Any): the checkpointer object used to save checkpoints.
    ...
    def step(self, iteration: int, **kwargs: Any):
        ...
        注1：如果有验证集，应根据验证性能保存模型, 验证性能存放在trainer的storage中(该操作在EvalHook中执行)
            又hook拥有trainer的弱引用，故在hook中可这样访问： 
            val_acc = self.trainer.storage._latest_scalars['val_acc'][0] (0~value, 1~iteration) 或
            val_avg = self.trainer.storage._history['val_acc'].global_avg() 或 ._global_avg
        注2：单独在PeriodicCheckpointer这个类中，self是该类的对象，不含trainer属性，但实际上该类作为基类被使用。
            在hooks.py中定义了"PeriodicCheckpointer(_PeriodicCheckpointer, HookBase)"，为方便理解，实际上是
            PeriodicCheckpointer_hook(PeriodicCheckpointer, HookBase), 
            PeriodicCheckpointer_hook重写了after_step()和before_train()，并从HookBase继承了after_train()和before_step()，
            从PeriodicCheckpointer继承了step()和save(), 所以PeriodicCheckpointer_hook的self.step()执行的是这里step()的代码，
            但是self拥有trainer属性，self形如: <fastreid.engine.hooks.PeriodicCheckpointer_hook object at 0x7ff56bcb35d0> 
        """
        iteration = int(iteration)
        additional_state = {"iteration": iteration}
        additional_state.update(kwargs)
        if (iteration + 1) % self.period == 0:
            self.checkpointer.save("model_{:07d}".format(iteration), **additional_state)  # self.checkpointer含有model属性, save()中可保存
