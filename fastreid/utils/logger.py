...
from datetime import datetime, timedelta    # 自增
...
def setup_logger(
    ...
    plain_formatter = logging.Formatter(
        "[%(asctime)s] %(name)s %(levelname)s: %(message)s", datefmt="%m/%d %H:%M:%S"   # "%Y-%m-%d_%H_%M_%S"
    ) 
    ...
    if distributed_rank == 0:
        ...
        if color:
            ...
            )   # 打印效果 [11/25 02:13:07 fastreid]: Rank of current process: 0. World size: 1
        else:
    ...
    if output is not None:
        ...
        if distributed_rank > 0:
            ...
            filename = os.path.splitext(filename)[0] + ".rank{}".format(distributed_rank) \
                       + os.path.splitext(filename)[1]    # 文件类型后缀(比如.txt)放最后
...
def _cached_log_stream(filename):
    return PathManager.open(filename, "a")  # "a"  将append改为write模式
