...
def default_setup(cfg, args):
    ...
    if comm.is_main_process() and output_dir:
        ...
        with PathManager.open(path, "w") as f:
            f.write(cfg.dump())
        logger.info("Full config saved to {}".format(os.path.abspath(path)))    # 保存当次实验的配置
    ...
    if not (hasattr(args, "eval_only") and args.eval_only): # 要么无此参数，要么此参数为False，执行下一行代码
...
class DefaultTrainer(SimpleTrainer):
    ...
    def build_hooks(self):
        ...
        ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_and_save_results))     # 直接是测试集，而非验证
    ...
    @classmethod
    def evaluation(cls, cfg, model):
        """ 若有专门的验证集，则可以仿test()写一个验证函数 """
        pass
