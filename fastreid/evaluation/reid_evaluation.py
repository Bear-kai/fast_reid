...
class ReidEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        if comm.get_world_size() > 1:
            comm.synchronize()
            features = comm.gather(self.features)   # return list
            features = sum(features, [])            # sum([[1,2],[3]], []) --> [1,2,3]
            ...
            # fmt: off
            if not comm.is_main_process():      # 只有主进程进行eval()
                return {}
            # fmt: on
        else:
            features = self.features            # list of tensor
            pids = self.pids
            camids = self.camids
