...
def eval_market1501(distmat, q_feats, g_feats, q_pids, g_pids, q_camids, g_camids, max_rank, use_distmat):
    ...
    if use_distmat:
        ...
        _, indices = index.search(q_feats, k=num_g)  # num_g, 仅返回部分结果，加快速度  max_rank*2

    # g_pids[indices]: num_q * dim, 该操作将indices中的底库样本排布序列号，转变为底库样本的ID标签
    matches = (g_pids[indices] == q_pids[:, np.newaxis]).astype(np.int32)
    ...
    for q_idx in range(num_q):
        ...
        order = indices[q_idx]
        remove = (g_pids[order] == q_pid) & (g_camids[order] == q_camid)
        keep = np.invert(remove)        # remove中的元素取反，即True变False，False变True
...
def evaluate_py(distmat, q_feats, g_feats, q_pids, g_pids, q_camids, g_camids, max_rank, use_metric_cuhk03, use_distmat):
