...
def get_parser():
    ...
    parser.add_argument(        ##
        ...
        default="./configs/Market1501/AGW_S50.yml", 
    ...
    parser.add_argument(
        "--rank-sort",          # 对各query的AP排序，ascending表示查看AP最低的若干个query
        default="ascending",
        help="rank order of visualization images by AP metric",
    )
    parser.add_argument(
        "--label-sort",         # 展示query对应的gallery照片，descending表示按相似度由大到小排列
    ...
    parser.add_argument(
        ...
        # default=['MODEL.WEIGHTS', './model_zoo/market_mgn_R50-ibn.pth'],        # 设置预训练模型
        default=['MODEL.WEIGHTS', './model_zoo/market_agw_S50.pth',
                 'MODEL.DEVICE', 'cuda:0'
                ],
...
if __name__ == '__main__':
    ...
    # 转换为np.ndarray, faiss不支持tensor
    cmc, all_ap, all_inp = evaluate_rank(distmat, q_feat.cpu().numpy(), g_feat.cpu().numpy(), q_pids, g_pids, q_camids, g_camids)
